...
int main(int argc, char *argv[]) {
	...
	while (1) {
		...
		double eye[3] = {6 * cos(2 * t), 7 * sin(2 * t), 2 + 1.5 * sq(sin(5 * t))};
		double coi[3] = {0, 0, 2};
		double up[3] = {eye[_X], eye[_Y], eye[_Z] + 1};
		double eye_spacing = 0.1;
		...
		object_reflectivity[objects] = 0.05/0.98;
...
//!!		objects++;
		...
		object_type[objects] = OBJ_CYLINDER;
		object_texture[objects] = TM_SOLID_COLOR;
		object_color[objects][_R] = 0.5;//0.64;
		object_color[objects][_G] = 0.8;//0.17;
		object_color[objects][_B] = 1;//0.17;
		object_reflectivity[objects] = 0.0;
		...
		for (int i = 0; i < num_bubbles; i++) {
			...
			T_type[T_n] = TZ;	T_param[T_n] = fmod((bubbles[i * 3 + 2] + (i % 10) * t / TAU), 1) * water_height;	T_n++;
			M3d_make_movement_sequence_matrix(object_matrix[objects], object_matrix_i[objects], T_n, T_type, T_param);
			objects++;
		}
		...
		M3d_make_movement_sequence_matrix(object_matrix[objects], object_matrix_i[objects], T_n, T_type, T_param);
		double straw_eye[3] = {0, glass_inner_radius - 3 * straw_radius, glass_thickness * 1.5};
		double straw_coi[3] = {0, -glass_inner_radius + 3 * straw_radius, glass_height + straw_radius};
...
//bad
		...
		object_reflectivity[objects] = 0.1;
		T_n = 0;
		T_type[T_n] = SX;	T_param[T_n] = 0.75;	T_n++;
		T_type[T_n] = SY;	T_param[T_n] = 0.75;	T_n++;
		T_type[T_n] = SZ;	T_param[T_n] = 0.75;	T_n++;
		T_type[T_n] = TZ;	T_param[T_n] = 0.75 * default_torus_parameters[1];	T_n++;
		...
		// Build napkin
		object_type[objects] = OBJ_PLANE;
		object_texture[objects] = TM_CLOCK;
		T_n = 0;
		T_type[T_n] = SX;	T_param[T_n] = 1;	T_n++;
		T_type[T_n] = SY;	T_param[T_n] = 1;	T_n++;
		T_type[T_n] = RZ;	T_param[T_n] = 35;	T_n++;
		T_type[T_n] = TX;	T_param[T_n] = 2;	T_n++;
		T_type[T_n] = TZ;	T_param[T_n] = 0.0000001;	T_n++;
		M3d_make_movement_sequence_matrix(object_matrix[objects], object_matrix_i[objects], T_n, T_type, T_param);
		objects++;
