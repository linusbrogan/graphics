...
// crazy number is to avoid conflict with same
// function in FPToolkit.c :
// the static moniker is apparently not protection against
// naming collisions when .c files are being included.
static void fputintB87732 (int x, FILE *fp)
{
   char *p ;
   int c[4],i ;
...
static int fgetintB87732 (FILE *fp)
{
   char *p ;
   int c[4],i ;
   int x ;
...
// otherwise returns the id number of the allocated xwd map
{
 ...
 header_size = fgetintB87732(fp) ;
 file_version = fgetintB87732(fp) ;
 format = fgetintB87732(fp) ;
 depth = fgetintB87732(fp) ;
 width =  fgetintB87732(fp) ;
 height =  fgetintB87732(fp) ;
 xoffset = fgetintB87732(fp) ;
 byte_order = fgetintB87732(fp) ;
 bitmap_unit = fgetintB87732(fp) ;
 bitmap_order = fgetintB87732(fp) ;
 bitmap_pad = fgetintB87732(fp) ;
 bits_per_pixel = fgetintB87732(fp) ;
 bytes_per_line = fgetintB87732(fp) ;
 visual_class = fgetintB87732(fp) ;
 redmask = fgetintB87732(fp) ;
 greenmask = fgetintB87732(fp) ;
 bluemask = fgetintB87732(fp) ;
 bitsperrgb = fgetintB87732(fp) ;
 numentriesincolormap = fgetintB87732(fp) ;
 numcolorstructures = fgetintB87732(fp) ;
 windowwidth = fgetintB87732(fp) ;
 windowheight = fgetintB87732(fp) ;
 windowx = fgetintB87732(fp) ;
 windowy = fgetintB87732(fp) ;
 windowborderwidth = fgetintB87732(fp) ;
...
// Always return 1
{
 ...
 header_size = 104 ;
 fputintB87732(header_size,fp) ;

 file_version = 7 ;
 fputintB87732(file_version,fp) ; 

 format = 2 ; // ZPixmap
 fputintB87732(format,fp) ;

 depth = 24 ;
 fputintB87732(depth,fp) ;

 width = xwdmap[mapid].width ;
 fputintB87732(width,fp) ;

 height = xwdmap[mapid].height ;
 fputintB87732(height,fp) ;

 xoffset = 0 ;
 fputintB87732(xoffset,fp) ;

 byte_order = 0 ;
 fputintB87732(byte_order,fp) ;

 bitmap_unit = 32 ;
 fputintB87732(bitmap_unit,fp) ;

 bitmap_order = 0 ;
 fputintB87732(bitmap_order,fp) ;

 bitmap_pad = 32  ;
 fputintB87732(bitmap_pad,fp) ;

 bits_per_pixel = 32 ;
 fputintB87732(bits_per_pixel,fp) ;

 bytes_per_line = width * 4 ;
 fputintB87732(bytes_per_line,fp) ;

 visual_class = 5 ; // more than one sample per pixel
 fputintB87732(visual_class,fp) ;

 redmask = 0x00ff0000 ;
 fputintB87732(redmask,fp) ;

 greenmask = 0x0000ff00 ;
 fputintB87732(greenmask,fp) ;

 bluemask = 0x000000ff ;
 fputintB87732(bluemask,fp) ;

 // bitsperrgb = 8 ; // hard to believe
 bitsperrgb = 24 ; 
 fputintB87732(bitsperrgb,fp) ;

 numentriesincolormap = 0 ; // not used :
 fputintB87732(numentriesincolormap,fp) ;

 numcolorstructures = 0 ;
 fputintB87732(numcolorstructures,fp) ;

 windowwidth = width ;
 fputintB87732(windowwidth,fp) ;

 windowheight = height ;
 fputintB87732(windowheight,fp) ;

 windowx = 0 ; // how crucial is this ?
 fputintB87732(windowx,fp) ;

 windowy = 0 ; // how crucial is this ?
 fputintB87732(windowy,fp) ;

 windowborderwidth = 0 ;
 fputintB87732(windowborderwidth,fp) ;
